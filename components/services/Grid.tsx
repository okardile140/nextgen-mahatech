...
export default function ServicesGrid() {
  ...
  const [services, setServices] = useState<ApiService[] | null>(staticServices);
      ...
      <div className="mx-auto max-w-7xl px-5 md:px-8">
        ...
        {services !== null && services.length === 0 ? (
          <p className="mx-auto max-w-xl text-center text-slate-500">
            Our service catalogue is being updated — please check back soon or contact us directly.
          </p>
        ) : (
                  ...
                  <div className={`rounded-3xl border border-slate-200/70 bg-gradient-to-br from-white to-slate-50 p-6 md:p-10 hover:shadow-2xl hover:shadow-indigo-500/10 transition-all ${i % 2 ? "md:flex-row-reverse" : ""} md:flex gap-8 items-start`}>
                    ...
                    <div className="flex-1 mt-4 md:mt-0 min-w-0">
                      <div className="flex items-center gap-3 flex-wrap">
                        <h3 className="text-xl md:text-2xl font-bold text-slate-900 break-words">{s.title}</h3>
                        {s.tagline ? (
                          <span className="text-xs font-semibold uppercase tracking-wider text-indigo-600 bg-indigo-50 px-3 py-1 rounded-full break-words">
                            {s.tagline}
                          </span>
                        ) : null}
                      </div>
                      <p className="mt-3 text-slate-600 leading-relaxed break-words">{s.longDescription}</p>
